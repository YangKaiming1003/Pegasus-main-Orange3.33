...
class OWDataTable(OWWidget):
    ...
    def _mmm_stan_model(self):
        ...
        plt.savefig(os.path.join(output_dir, "EDA-correlation.png"))
        plt.figure(figsize=(50,50))
        sns.pairplot(df[media_impression+['sales']], vars=media_impression+['sales'])
        plt.savefig(os.path.join(output_dir, "EDA-distribution.png"))
        ...
        for i in range(len(mmm['media_vars'])):
            ...
            ax.set_title(md)
            plt.savefig(os.path.join(output_dir, "Media-coefficient-distribution.png"))
        ...
        for i in range(len(hm_keys)):
            ...
            ax.set_title(md)
            plt.savefig(os.path.join(output_dir, "EC-distribution.png"))
        f = plt.figure(figsize=(18,12))
        hm_keys = list(hill_models.keys())
        for i in range(len(hm_keys)):
            ax = f.add_subplot(4,3,i+1)
            ...
            ax.axvline(median_x, color='g', linestyle='-', alpha=0.5)
            ax.set_title(md)
            plt.savefig(os.path.join(output_dir, "Slope-distribution.png"))
        ...
        for i in range(len(hm_keys)):
            ...
            ax.set_title(md)
            plt.savefig(os.path.join(output_dir, "Fitted-hill.png"))
        ms_df = pd.DataFrame()
        for md in list(hill_models.keys()):
            hill_model = hill_models[md]
        ...
        for i in range(len(weekly_roas.columns)):
            ...
            ax.set(xlabel=None)
            ax.set_title(md)
            plt.savefig(os.path.join(output_dir, "Weekly-Roas-distribution.png"))
        f = plt.figure(figsize=(18,12))
        for i in range(len(weekly_roas.columns)):
            md = weekly_roas.columns[i]
            ax = f.add_subplot(4,3,i+1)
            ...
            ax.set(xlabel=None)
            ax.set_title(md)
            plt.savefig(os.path.join(output_dir, "Weekly-Roas-distribution_past_1year.png"))
